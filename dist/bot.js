...
class MessageStore {
    constructor() {
        this.messages = [];
        this.maxMessages = MAX_STORED_MESSAGES;
    }
...
function registerChatTools(server, bot) {
    ...
    server.tool("read-chat", "Get recent chat messages from players", {
...
// ========== Flight Tools ==========
function registerFlightTools(server, bot) {
    server.tool("fly-to", "Make the bot fly to a specific position", {
        x: z.number().describe("X coordinate"),
        y: z.number().describe("Y coordinate"),
        z: z.number().describe("Z coordinate")
    }, async ({ x, y, z }) => {
        ...
        try {
            ...
            if (controller.signal.aborted) {
                ...
                return createErrorResponse(`Flight timed out after ${FLIGHT_TIMEOUT_MS / 1000} seconds. The destination may be unreachable. ` +
...
function getBlockProperties(block) {
    ...
    if (block.material)
        properties.push(`Material: ${block.material}`);
    if (block.metadata !== undefined)
        properties.push(`Metadata: ${block.metadata}`);
    if (block.biome)
        properties.push(`Biome: ${block.biome}`);
    ...
    const activeStatus = [];
    if (block.blockData?.lit === true)
        activeStatus.push('LIT');
    if (block.blockData?.powered === true)
        activeStatus.push('POWERED');
    if (block.blockData?.open === true)
        activeStatus.push('OPEN');
    if (block.blockData?.extended === true)
        activeStatus.push('EXTENDED');
    if (block.blockData?.triggered === true)
        activeStatus.push('TRIGGERED');
    if (block.blockData?.activated === true)
        activeStatus.push('ACTIVATED');
    if (activeStatus.length > 0) {
        properties.push(`Active Status: ${activeStatus.join(', ')}`);
    }
    return properties.join('\n');
}
function registerObservationTools(server, bot) {
    // New tool: Get single block with all properties
    server.tool("get-block-at", "Get detailed information about a single block at specific coordinates using blockAt function", {
        x: z.number().describe("X coordinate"),
        y: z.number().describe("Y coordinate"),
        z: z.number().describe("Z coordinate"),
        includeExtraInfo: z.boolean().optional().describe("Include extra block entity information (slower)").default(true)
    }, async ({ x, y, z, includeExtraInfo }) => {
        try {
    ...
    server.tool("get-blocks-in-area", "Get all blocks within a 3D rectangular area defined by two corner positions with detailed properties", {
    ...
    }, async ({ x1, y1, z1, x2, y2, z2, filterType, includeProperties, maxBlocks }) => {
        try {
            ...
            for (let x = minX; x <= maxX; x++) {
                for (let y = minY; y <= maxY; y++) {
                    for (let z = minZ; z <= maxZ; z++) {
                        ...
                        if (!block)
                            continue;
                        if (filterType && block.name !== filterType)
                            continue;
                        const blockInfo = {
    ...
    server.tool("get-blocks-in-radius", "Get all blocks within a spherical radius from a center point with detailed properties", {
        ...
        includeProperties: z.boolean().optional().describe("Include detailed block properties").default(false),
        maxBlocks: z.number().optional().describe("Maximum number of blocks to return details for").default(50)
    }, async ({ x, y, z, radius, filterType, includeProperties, maxBlocks }) => {
        try {
            const MAX_RADIUS = 32;
    ...
    server.tool("scan-layers", "Scan horizontal layers at specific Y levels to find blocks with detailed properties", {
        ...
        includeProperties: z.boolean().optional().describe("Include detailed block properties").default(false),
        maxBlocks: z.number().optional().describe("Maximum number of blocks to return details for").default(50)
    }, async ({ centerX, centerZ, yLevel, radius, filterType, includeProperties, maxBlocks }) => {
        try {
            const MAX_RADIUS = 32;
            if (radius > MAX_RADIUS) {
                return createResponse(`Radius too large (${radius}). Maximum allowed: ${MAX_RADIUS} blocks.`);
...
// ========== Game State Tools ============
function registerGameStateTools(server, bot) {
    server.tool("detect-gamemode", "Detect the gamemode on game", {}, async () => {
        try {
